<?php /*a:2:{s:62:"/www/wwwroot/tdata.tgbota.top/app/admin/view/mtcate/index.html";i:1766469387;s:66:"/www/wwwroot/tdata.tgbota.top/app/admin/view/common/container.html";i:1729069940;}*/ ?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<el-table :row-class-name="rowClass" @selection-change="selection"  @row-click="handleRowClick" :tree-props="{children: 'children'}"  :default-expand-all="expand" row-key="class_id"  :header-cell-style="{ background: '#eef1f6', color: '#606266' }" v-loading="loading"  ref="multipleTable" border class="eltable" :data="list"  style="width: 100%">
	...
	<el-table-column align="left"  property="class_name" label="分类名称" show-overflow-tooltip width="">
	</el-table-column>
	...
	<el-table-column align="center"  property="user_count" label="账户数量" >
	</el-table-column>
	<el-table-column align="left"  property="task_num" label="任务号组" show-overflow-tooltip width="">
	</el-table-column>
	...
	<el-table-column :fixed="ismobile()?false:'right'" label="操作" align="center" width="180">
		<template slot-scope="scope">
			<div v-if="scope.row.class_id">
				<el-button v-if="checkPermission('/admin/Mtcate/update.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-edit" type="primary" @click="update(scope.row)" >修改</el-button>
				<el-button v-if="checkPermission('/admin/Mtcate/delete.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-delete" type="danger" @click="del(scope.row)" >删除</el-button>
			</div>
		</template>
	</el-table-column>
</el-table>
...
<script src="/components/admin/mtcate/add.js?v=<?php echo rand(1000,9999)?>"></script>
<script src="/components/admin/mtcate/update.js?v=<?php echo rand(1000,9999)?>"></script>
<script>
new Vue({
	el: '#app',
	components:{
	...
	data: function() {
		return {
			...
			searchData:{},
			button_group:[
				{name:'添加',color:'success',access:'/admin/Mtcate/add.html',icon:'el-icon-plus',disabled:'',clickname:'add'},				
				{name:'删除',color:'danger',access:'/admin/Mtcate/delete.html',icon:'el-icon-delete',disabled:'single',clickname:'del'},
			],
			loading: false,
			page_data: {
				limit: 20,
	...
	methods:{
		index(){
			...
			this.loading = true
			axios.post(base_url + '/Mtcate/index',param).then(res => {
				if(res.data.status == 200){
		...
		updateExt(row,field){
			if(row.class_id){
				axios.post(base_url + '/Mtcate/updateExt',{class_id:row.class_id,[field]:row[field]}).then(res => {
					if(res.data.status == 200){
		...
		update(row){
			...
			axios.post(base_url + '/Mtcate/getUpdateInfo',{class_id:id}).then(res => {
				if(res.data.status == 200){
		...
		del(row){
			this.$confirm('确定操作吗?', '提示', {
				...
				axios.post(base_url + '/Mtcate/delete',{class_id:ids}).then(res => {
					if(res.data.status == 200){
						this.$message({message: res.data.msg, type: 'success'})
						this.index()
					}else{
