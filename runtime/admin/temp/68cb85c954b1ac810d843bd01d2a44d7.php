<?php /*a:2:{s:64:"/www/wwwroot/tdata.tgbota.top/app/admin/view/mttask/addtask.html";i:1765540648;s:66:"/www/wwwroot/tdata.tgbota.top/app/admin/view/common/container.html";i:1729069940;}*/ ?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
              ...
              <el-row>
                <el-col>
                  <el-form-item label="群列表" prop="group_list">
                    <el-input v-model="form.group_list" placeholder="请输入群列表，多个群用;逗号分隔"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                  ...
                  <el-row :gutter="10">
                    ...
                    <el-col :span="3" >
                      <el-select v-model="message.feedbackType" placeholder="回复" style="width: 100%;" :disabled="index == 0&&currentPage==1"  @visible-change="onFeedbackTypeChange(message, index + (currentPage-1)*pageSize)">
                        <el-option label="不回复" value="0"></el-option>
                        <el-option
                            v-for="k in cindex"
                            :label="'回复' + k"
                            :value="k"
                        ...
                        <el-row :gutter="10">
                          ...
                          <el-col :span="24" v-if="message.sendType === 'text' || message.sendType === 'image_text'">
                            <el-form-item :prop="'messages.' + (index + (currentPage-1)*pageSize) + '.text'" :rules="{required: message.sendType === 'text' || message.sendType === 'image_text', message: '请输入文本内容', trigger: 'blur'}">
                              <el-input v-model="message.text" type="textarea" :rows="2" placeholder="请输入文本内容"></el-input>
                            </el-form-item>
                          </el-col>
                          ...
                          <el-col :span="24" v-if="message.sendType === 'image' || message.sendType === 'image_text'">
                            <el-form-item :prop="'messages.' + (index + (currentPage-1)*pageSize) + '.images'" :rules="{required: message.sendType === 'image' || message.sendType === 'image_text', message: '请上传图片', trigger: 'change'}">
                              ...
                              <Upload  size="small"  file_type="images" upload_type = '2'  :images.sync="message.images"></Upload>
                            </el-form-item>                
                          </el-col>
                    ...
                    <el-col :span="2">
                      <el-input v-model.number="message.delay" type="number" placeholder="延时(秒)" min="1" max="1000" 
                              style="width: 100%;"></el-input>
                    </el-col>
                    <el-col :span="2">
                      <el-button type="danger" @click="deleteMessage(index + (currentPage-1)*pageSize)" size="small" v-if="index + (currentPage-1)*pageSize > 0">
      ...
      data() {
        return {
          ...
          rules: {
            ...
            group_list: [
              { required: true, message: '请输入群列表', trigger: 'blur' }
            ],
            concurrent: [
      ...
      methods: {
        ...
        fetchFormData() {
          axios.post(base_url + '/Baseconfig/getInfo').then(res => {
            if(res.data.status == 200){
              this.form = JSON.stringify(res.data) == '[]' ? this.form : res.data.data;
        ...
        beforeImport(file) {
            ...
            if (!isTxt && !isJson) {
              this.$message.error('请上传txt或json格式文件!');
              return false;
            }
            return true;
        },
