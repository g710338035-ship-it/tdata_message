<?php /*a:2:{s:66:"/www/wwwroot/tdata.tgbota.top/app/admin/view/socktscate/index.html";i:1751466340;s:66:"/www/wwwroot/tdata.tgbota.top/app/admin/view/common/container.html";i:1729069940;}*/ ?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
	...
	<el-table-column :fixed="ismobile()?false:'right'" label="操作" align="center" width="180">
		<template slot-scope="scope">
			<div v-if="scope.row.class_id">
				<el-button v-if="checkPermission('/admin/Socktscate/update.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-edit" type="primary" @click="update(scope.row)" >修改</el-button>
				<el-button v-if="checkPermission('/admin/Socktscate/delete.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-delete" type="danger" @click="del(scope.row)" >删除</el-button>
			</div>
		</template>
	</el-table-column>
</el-table>
...
<script src="/components/admin/socktscate/add.js?v=<?php echo rand(1000,9999)?>"></script>
<script src="/components/admin/socktscate/update.js?v=<?php echo rand(1000,9999)?>"></script>
<script>
new Vue({
	el: '#app',
	components:{
	...
	data: function() {
		return {
			...
			searchData:{},
			button_group:[
				{name:'添加',color:'success',access:'/admin/Socktscate/add.html',icon:'el-icon-plus',disabled:'',clickname:'add'},				
				{name:'删除',color:'danger',access:'/admin/Socktscate/delete.html',icon:'el-icon-delete',disabled:'multiple',clickname:'del'},
			],
			loading: false,
			page_data: {
				limit: 20,
	...
	methods:{
		index(){
			...
			this.loading = true
			axios.post(base_url + '/Socktscate/index',param).then(res => {
				if(res.data.status == 200){
		...
		updateExt(row,field){
			if(row.class_id){
				axios.post(base_url + '/Socktscate/updateExt',{class_id:row.class_id,[field]:row[field]}).then(res => {
					if(res.data.status == 200){
		...
		update(row){
			...
			axios.post(base_url + '/Socktscate/getUpdateInfo',{class_id:id}).then(res => {
				if(res.data.status == 200){
		...
		del(row){
			this.$confirm('确定操作吗?', '提示', {
				...
				axios.post(base_url + '/Socktscate/delete',{class_id:ids}).then(res => {
					if(res.data.status == 200){
						this.$message({message: res.data.msg, type: 'success'})
						this.index()
					}else{
