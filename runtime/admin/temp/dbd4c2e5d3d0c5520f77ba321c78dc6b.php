<?php /*a:2:{s:60:"/www/wwwroot/tdata.tgbota.top/app/admin/view/role/index.html";i:1635492092;s:66:"/www/wwwroot/tdata.tgbota.top/app/admin/view/common/container.html";i:1729069940;}*/ ?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<div v-if="search_visible" id="search" class="search">
	<el-form ref="form" size="small" :model="searchData" inline>
		<el-form-item label="角色名称">
			<el-input id="name" v-model="searchData.name"  style="width:150px;" placeholder="请输入角色名称"></el-input>
		</el-form-item>
...
<el-table :row-class-name="rowClass" @selection-change="selection"  @row-click="handleRowClick"  row-key="role_id"  :header-cell-style="{ background: '#eef1f6', color: '#606266' }" v-loading="loading"  ref="multipleTable" border class="eltable" :data="list"  style="width: 100%">
	<el-table-column align="center" type="selection" width="42"></el-table-column>
	<el-table-column align="center" type = '' property="role_id" label="编号" show-overflow-tooltip width="70">
	</el-table-column>
	<el-table-column align="center"  property="name" label="角色名称" show-overflow-tooltip width="">
	</el-table-column>
	...
	<el-table-column :fixed="ismobile()?false:'right'" label="操作" align="center" width="180">
		<template slot-scope="scope">
			<div v-if="scope.row.role_id">
				<el-button v-if="checkPermission('/admin/Role/update.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-edit" type="primary" @click="update(scope.row)" >修改</el-button>
				<el-button v-if="checkPermission('/admin/Role/delete.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-delete" type="danger" @click="del(scope.row)" >删除</el-button>
			</div>
		</template>
	</el-table-column>
</el-table>
...
<script src="/components/admin/role/add.js?v=<?php echo rand(1000,9999)?>"></script>
<script src="/components/admin/role/update.js?v=<?php echo rand(1000,9999)?>"></script>
<script>
new Vue({
	el: '#app',
	components:{
	...
	data: function() {
		return {
			...
			searchData:{},
			button_group:[
				{name:'添加',color:'success',access:'/admin/Role/add.html',icon:'el-icon-plus',disabled:'',clickname:'add'},
				{name:'修改',color:'primary',access:'/admin/Role/update.html',icon:'el-icon-edit',disabled:'single',clickname:'update'},
				{name:'删除',color:'danger',access:'/admin/Role/delete.html',icon:'el-icon-delete',disabled:'multiple',clickname:'del'},
			],
			loading: false,
			page_data: {
				limit: 20,
	...
	methods:{
		index(){
			...
			this.loading = true
			axios.post(base_url + '/Role/index',param).then(res => {
				if(res.data.status == 200){
		...
		updateExt(row,field){
			if(row.role_id){
				axios.post(base_url + '/Role/updateExt',{role_id:row.role_id,[field]:row[field]}).then(res => {
					if(res.data.status == 200){
		...
		update(row){
			...
			axios.post(base_url + '/Role/getUpdateInfo',{role_id:id}).then(res => {
				if(res.data.status == 200){
		...
		del(row){
			this.$confirm('确定操作吗?', '提示', {
				...
				axios.post(base_url + '/Role/delete',{role_id:ids}).then(res => {
					if(res.data.status == 200){
						this.$message({message: res.data.msg, type: 'success'})
						this.index()
					}else{
