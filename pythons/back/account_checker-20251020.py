...
async def batch_check_account(request: Request):
    ...
    try:
        ...
        # 并发处理所有账号（使用信号量控制并发量）
        max_concurrent = min(10, len(accounts))  # 动态调整最大并发数
        semaphore = asyncio.Semaphore(max_concurrent)
        
        async def bounded_task(account):
            async with semaphore:
                return await process_single_account(
        ...
        # 创建并执行所有任务
        tasks = [bounded_task(account) for account in accounts]
...
# -------------- 新增：单账号处理辅助函数（批量接口依赖）--------------
async def process_single_account(tdata_path, tdata_phone, account_id):
    """异步接口，通过进程池调用同步处理函数"""
    loop = asyncio.get_event_loop()
    # 使用进程池执行CPU密集型任务，避免GIL限制
