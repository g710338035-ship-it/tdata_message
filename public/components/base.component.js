...
Vue.component('Upload', {
	...
	<div>
		<div v-if="file_type == 'images'" class="image-list" style="width:100%">
			...
			<!-- 新增：多图上传时的从服务器选择按钮 -->
			<el-row v-if="upload_type == '2'" style="margin-top: 10px;">
				<el-col :span="24">
					<el-tooltip class="item" effect="dark" content="从服务器选择图片" placement="top">
						<el-button class="el-icon-picture" size="small" @click="openImagesPickForMultiple">从服务器选择图片</el-button>
					</el-tooltip>
				</el-col>
			</el-row>
		</div>
		<div v-else-if="file_type == 'image'" class="image-list" style="width:100%">
			<el-row v-if="upload_type == '1'">
				...
				<el-upload action="#" ref="upload" class="image-uploader"  :http-request="upload">
					<i :class="loading ? 'el-icon-loading' : 'el-icon-plus'"></i>
				 </el-upload>
			</el-row>
			<el-row v-else>
		...
		<div v-else-if="file_type == 'file'" class="image-list" style="width:100%">
			...
			<ul class="el-upload-list el-upload-list--text">
				<li v-if="sigleFile" tabindex="0" class="el-upload-list__item is-success">
					<a @click.stop="handleRemove('file')" class="el-upload-list__item-name"><i class="el-icon-document"></i>{{sigleFile}}</a>
		...
		<ImagesPick 
			:imagesDiaShow.sync="imagesDialogStatus" 
			:siglepic.sync ="siglepic" 
			:multiple="imagesPickMultiple"
	...
	components:{
		ImagesPick:{
			...
			methods:{
				...
				submitPick(){
					if(this.multiple) {
						// 多选模式，触发自定义事件传递选中的图片数组
						this.$emit('images-selected', this.files)
					} else {
						// 单选模式，更新siglepic
	...
	data() {
		return {
			...
			// 修改：移除selectedImages，简化逻辑
			imagesPickMultiple: false, // 控制图片选择器是多选还是单选
		};
	},
	methods:{
		beforeUpload(item){
		...
		openImagesPick(){
			// 打开单选图片选择器（用于单图上传）
			this.imagesPickMultiple = false
			this.imagesDialogStatus = true
		},
		openImagesPickForMultiple(){
			// 打开多选图片选择器（用于多图上传）
			this.imagesPickMultiple = true
			this.imagesDialogStatus = true
		},
		changeSort(){
			this.$emit('update:images',this.imageList)
		},
		// 新增方法：处理从图片选择器返回的图片数组
		handleImagesSelected(selectedImages) {
			if(selectedImages && selectedImages.length > 0) {
				selectedImages.forEach(filepath => {
					// 检查是否已存在相同的图片
					const exists = this.imageList.some(item => item.url === filepath)
					if(!exists) {
						// 从路径中提取文件名
						const filename = filepath.split('/').pop() || '未命名图片'
						this.imageList.push({
							url: filepath,
							name: filename
						})
...
Vue.component('Page', {
    ...
    <div :class="{'hidden':hidden}" class="pagination-container">
      ...
      <div style="margin-left:10px;">
        ...
        <el-input
          ...
          size="mini"
          style="width:70px;"
          @keyup.enter.native="applyCustomSize"
  ...
  data() {
    return {
      customSize: this.limit,
      // 内部维护的pageSizes，支持动态添加
  ...
  watch: {
    limit(newVal) {
      this.customSize = newVal;
      this.addToPageSizesIfNeeded(newVal);
    }
  },
  methods: {
    handleSizeChange(val) {
      this.customSize = val;
      this.addToPageSizesIfNeeded(val);
    ...
    applyCustomSize() {
      // 验证输入
      if (!this.customSize || this.customSize < 1) {
        this.customSize = this.limit;
        return;
      }
      
      // 限制最大值，避免过大值导致性能问题
      const validSize = Math.min(Math.max(1, this.customSize), 5000);
      
      // 如果输入的值不在pageSizes中，则添加到pageSizes
      this.addToPageSizesIfNeeded(validSize);
      
      // 更新pageSize
      this.pageSize = validSize;
      this.$emit('pagination', { page: 1, limit: validSize })
    },
    addToPageSizesIfNeeded(size) {
      const sizeNum = Number(size);
      if (!this.internalPageSizes.includes(sizeNum)) {
        // 添加到pageSizes并排序
        this.internalPageSizes.push(sizeNum);
        this.internalPageSizes.sort((a, b) => a - b);
        
        // 如果数组太长，保留前10个常用值
        if (this.internalPageSizes.length > 10) {
          this.internalPageSizes = this.internalPageSizes.slice(0, 10);
        }
      }
    }
  }
