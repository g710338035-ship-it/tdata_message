	...
	template: `
		<el-dialog title="修改" width="700px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
			<el-form :size="size" ref="form" :model="form" :rules="rules" :label-width=" ismobile()?'90px':'16%'">
				...
				<el-row >
					<el-col :span="24">
						<el-form-item label="选择群标签" prop="mp_id">
                    ...
                    <el-form-item label="过滤值" prop="filterValue">
                      <template v-if="form.filterType === 'user_id'">
                        <el-input type="textarea"
                          ...
                          autoComplete="off" 
                          clearable 
                          placeholder="请输入用户 ID，多个 ID 用逗号分隔">
                        </el-input>
                      </template>
                      <template v-else-if="form.filterType === 'keyword'">
                        <el-input type="textarea"
                          ...
                          autoComplete="off" 
                          clearable 
                          placeholder="请输入关键词，多个关键词用中文逗号分隔">
                        </el-input>
                      </template>
                      <template v-else>
                        <el-input type="textarea"
	...
	methods: {
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Mprenwu/update',this.form).then(res => {
