Vue.component('Update', {
	template: `
		<el-dialog title="修改" width="600px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
			<el-form :size="size" ref="form" :model="form" :rules="rules" :label-width=" ismobile()?'90px':'16%'">
			    ...
			    <el-row >
					<el-col :span="24">
						<el-form-item label="任务号组" prop="task_num">
							<el-input  v-model="form.task_num" autoComplete="off" clearable  placeholder="请输入任务号组账户数量"></el-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row >
	...
	watch:{
		show(val){
			if(val){
				axios.post(base_url + '/Mtcate/getFieldList').then(res => {
	...
	methods: {
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Mtcate/update',this.form).then(res => {
						if(res.data.status == 200){
