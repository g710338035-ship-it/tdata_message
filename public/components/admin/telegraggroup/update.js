	...
	template: `
		<el-dialog title="修改" width="600px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
			<el-form :size="size" ref="form" :model="form" :rules="rules" :label-width=" ismobile()?'90px':'16%'">
				<el-row >
					<el-col :span="24">
						<el-form-item label="群名称" prop="title">
							<el-input  v-model="form.title" autoComplete="off" clearable  placeholder="请输入用户名"></el-input>
						</el-form-item>
					</el-col>
				</el-row>
				...
				<el-row >
					<el-col :span="24">
						<el-form-item label="头像" prop="group_image">
							<Upload v-if="show" size="small"  upload_type="1" file_type="image" :image.sync="form.group_image"></Upload>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row>
						<el-col :span="24">
							<el-form-item label="欢迎语" prop="content">
								<el-input  type="textarea" autoComplete="off" v-model="form.content"  :autosize="{ minRows: 2, maxRows: 4}" clearable placeholder="请输入欢迎语"/>
							</el-form-item>
						</el-col>
					</el-row>
				<el-row >
	...
	methods: {
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Telegraggroup/update',this.form).then(res => {
