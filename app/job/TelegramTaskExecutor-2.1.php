...
class TelegramTaskExecutor
{
        ...
        switch ($result['action'] ?? '') {
            ...
            case 'continue':
                if (isset($result['nextIndex'])) {
                    ...
                    // 设置检查标记，60秒过期（大于延迟时间）
                    $this->redis->setex($taskCheckKey, $delay + 5, $taskInstanceId);
    ...
    private function findAlternativeUser(int $taskId, int $excludeUserId): ?array
    {
        try {
            ...
            // 如果任务中有多个消息，已经使用的账户ID可能不止一个
            // 将排除的用户ID列表合并
            $excludeUserIds = array_merge([$excludeUserId], $usedUserIds);
            ...
            $user = Db::name('mtuser')
                ...
                ->where('session_path', '<>', '')
                ->order('id asc') // 简单按ID排序，取第一个
                ->field('id, account, cateid, tdata_path, session_path, proxyip, archive, account_status, account_status_desc, last_api_address')
                ->find();
            
            if ($user) {               
                Log::info("从分类 {$accountGroup} 找到可用账户: {$user['id']} - {$user['account']}");
                return $user;
            }
            
            Log::warning("分类 {$accountGroup} 下没有可用的备用账户（排除用户: {$excludeUserId}），尝试从所有账户中查找");
            return null;
    ...
    private function updateCacheAfterUserReplacement(int $taskId, int $oldUserId, int $newUserId, array $affectedIndexes): void
    {
        try {
            ...
            $this->redis->delete($oldUserCacheKey);
    ...
    private function handleTaskCompletion($task, int $taskId, int $xhnum, int $currentCycle,string $taskConfigCacheKey, string $statusKey,string $lastProcessedKey, array $groupList): array
    {
        ...
        if ($xhnum > 1 && $currentCycle < $xhnum) {
            ...
            $this->redis->delete($this->getRedisKey("cycle:{$taskId}"));//删除当前轮次缓存
    ...
    private function handleSingleMessage(int $taskId, array $messages, array $groupList, array $userMap, int $index): array
    {
        ...
        if (($sendUser['account_status'] ?? '') == '退出'||$sendUser['last_api_address'] == '') {
        ...
        foreach ($groupList as $groupId) {
            ...
            $messageId = $this->getMessageIdFromCache($taskId, $groupId, $index);
            if ($messageId) {
                // 标记为已发送
    ...
    private function getMessageIdFromCache(int $taskId, string $groupId, int $index): ?string
    {
        $messageIdsKey = $this->getMessageIdsCacheKey($taskId, $groupId);
    ...
    private function sendMessageToGroup(int $taskId, array $msgConfig, array $sendUser, string $groupId, int $index): array
    {
        try {
            // 生成唯一消息ID，用于幂等性
            $messageUniqueId = md5("{$taskId}_{$groupId}_{$index}_{$sendUser['id']}");
            ...
            if ($result['status'] ?? false) {
                ...
                if ($hasExpiredLinkError) {
                    $this->writeTaskLog($taskId, "{$sendUser['account']} [{$index}]{$msgConfig['text']} 1 {$groupId} 发送失败 邀请链接已过期,请检查账户是否在该群组");
                    return [false, '无权限'];
                }
                ...
                if($messageId){
                    ...
                    $this->writeTaskLog($taskId, "{$sendUser['account']} [{$index}]{$msgConfig['text']} 1 {$groupId}  发送失败 ");
                    return [false, $error];
                }
            } else {
                $error = "群组 {$groupId} 消息发送失败: " . ($result['message'] ?? '未知错误');
