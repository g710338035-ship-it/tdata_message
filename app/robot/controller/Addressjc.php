...
class Addressjc extends Apibot
{
    protected $cacheBot;
        ...
        switch ($text) {
            case '/addressjc_set':
		        $bot=Cache::store('redis')->get($this->cacheBot);
                    ...
                    if ($bot['addressjc'] == 1) {
                        $str = '已*【开启】*地址监测功能，如果需要 * 取消检测 * 请点击下方按钮'; // 使用 Markdown 格式
                        $bs = '取消❌';
                    } else {
                        $str = '已*【关闭】*地址监测功能，如果需要 * 开启检测 * 请点击下方按钮'; // 使用 Markdown 格式
                        $bs = '开启✅';
                    }
                    ...
                    $bwButtons[] = [[
                        'text' => "💠$bs",
                        'callback_data' => '/addressjc-status'
                    ],  [
                        'text' => '🔙 返回',
                        'callback_data' => '/start'
                    ]];
		        ...
		        break;
		    case '/addressjc-status':
		        $bot=Cache::store('redis')->get($this->cacheBot);
		        $addressjc = ($bot['addressjc'] == 0) ? 1 : 0;
                // 更新 is_top 值
                Db::name('telegrambot')->where('bot_token', $token)->update(['addressjc' => $addressjc]);
		        
		   
                    if ($bot['addressjc'] == 0) {
                        $str = '已*【开启】*地址监测功能，如果需要 * 取消 * 请点击下方按钮'; // 使用 Markdown 格式
                        $bs = '取消❌';
                    } else {
                        $str = '已*【关闭】*地址监测功能，如果需要 * 开启 * 请点击下方按钮'; // 使用 Markdown 格式
                        $bs = '开启✅';
                    }
