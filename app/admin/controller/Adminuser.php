...
class Adminuser extends Admin {
	...
	function index(){
		if (!$this->request->isPost()){
			...
			$res = AdminuserModel::where(formatWhere($where))->field($field)->withJoin($withJoin,'left')->order('user_id desc')->paginate(['list_rows'=>$limit,'page'=>$page])->toArray();
	...
	public function add(){
		...
		return json(['status'=>200,'data'=>$res->user_id,'msg'=>'添加成功']);
	}
	...
	private function getSqlField($list){
		...
		if(in_array('role_id',explode(',',$list))){
			$data['role_ids'] = $this->query('select role_id,name from pre_role');
		}
		return $data;
	}
