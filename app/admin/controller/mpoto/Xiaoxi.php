...
use app\admin\model\Xiaoxi as XiaoxiModel;
use think\facade\Db;

class Xiaoxi extends Admin {
	...
	function index(){
		...
		/*	$withJoin = [
			...
			$res = XiaoxiModel::where(formatWhere($where))->order('id desc')->paginate(['list_rows'=>$limit,'page'=>$page])->toArray();
	...
	function delete(){
		...
		XiaoxiModel::destroy(['id'=>explode(',',$idx)],true);
		return json(['status'=>200,'msg'=>'操作成功']);
	}
	...
	public function forbidden(){
		...
		$data['status'] = '0';
		$res = XiaoxiModel::field('status')->where(['id'=>explode(',',$idx)])->update($data);
		return json(['status'=>200,'msg'=>'操作成功']);
	}
