...
use app\admin\model\Renwu as RenwuModel;
use think\facade\Db;
...
class Renwu extends Admin {
	...
	function index(){
		...
		/*	$withJoin = [
			...
			$res = RenwuModel::where(formatWhere($where))->order('id desc')->paginate(['list_rows'=>$limit,'page'=>$page])->toArray();
	...
	function delete(){
		...
		RenwuModel::destroy(['id'=>explode(',',$idx)],true);
		return json(['status'=>200,'msg'=>'操作成功']);
	}
	...
	public function forbidden(){
		...
		$data['status'] = '0';
		$res = RenwuModel::field('status')->where(['id'=>explode(',',$idx)])->update($data);
		return json(['status'=>200,'msg'=>'操作成功']);
	}
