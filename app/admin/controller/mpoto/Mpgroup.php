...
use app\admin\model\Mpgroup as MpgroupModel;
use app\admin\model\Monitorphone as MonitorphoneModel;
use think\facade\Db;
use think\facade\Log;
use think\facade\Queue;
class Mpgroup extends Admin {
	...
	function index(){
		if (!$this->request->isPost()){
			...
			$where['group_name'] = $this->request->post('username', '', 'serach_in');
			...
			$res = MpgroupModel::where(formatWhere($where))->order('id desc')->paginate(['list_rows'=>$limit,'page'=>$page])->toArray();
	...
	public function update(){
        ...
        $originalData = MpgroupModel::find($data['id']);
	...
	function delete(){
		...
		MpgroupModel::destroy(['id'=>explode(',',$idx)],true);
	...
	public function forbidden(){
		...
		$data['status'] = '0';
		$res = MpgroupModel::field('status')->where(['id'=>explode(',',$idx)])->update($data);
		return json(['status'=>200,'msg'=>'操作成功']);
	}
    ...
    public function getRobot_id(){
		...
		$data = $this->getSelectPageData('select id,group_name from cd_mpgroup',$where,$skip); 
