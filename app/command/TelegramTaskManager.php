...
{
    $client = new Client();
    $pythonServiceUrl = config('telegram.python_service_url'); // 配置Python服务地址
 ...
 * 处理后续消息发送（HTTP版本）
 */
private function handleSubsequentMessages($taskId, $messages, $groupList, $firstMessageIds, $userMap)
{
    $successCount = 0;
    $failCount = 0;
    $failDetails = [];
        ...
        foreach ($groupList as $groupId) {
            ...
            try {
                ...
                if ($result['status']) {
                    ...
                    $failCount++;
                    $failDetails[] = "群组 {$groupId} 消息发送失败: " . ($result['error'] ?? '未知错误');
                }
            } catch (\Exception $e) {
                $failCount++;
