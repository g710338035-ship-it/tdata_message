...
class Mpapi extends BaseController
{
    ...
    private function handleNewMessage($update, $accountName, $instance)
    {
        ...
        if (isset($update['message']['action']) && $update['message']['action']['_'] === 'messageActionChatMigrateTo') {
            $text='群组已迁移至频道';
        }
           // log::info($text);
        if ($text && $user_id&& $user_id>0&&$chat_id<0) {
           // log::info($text);
            ...
            // 存储用户信息到数据库
            $isalluser=$this->saveUserInfo($user_id, $userInfo['username'], $userInfo['nickname'], $text, null, $message_link, $accountName);
            // 保存群组信息到数据库
           
            $this->saveGroupInfo($chat_id,  $accountName, $instance);
            if (isset($update['message']['action']) && $update['message']['action']['_'] === 'messageActionChatMigrateTo') {
                $channelId = $update['message']['action']['channel_id'];
                //log::info("群组已迁移至频道，原ID".$chat_id."新频道 ID 为: ". $channelId);
                // 这里可以添加更多逻辑，例如更新本地数据库中关于该群组和频道的信息等
                    try {
                    Db::startTrans();
    
                    $groupType = 'supergroup';
                    $resultGroup = Db::name('mpgroup')
                    ...
                    if ($resultGroup === false) {
                        throw new \Exception('更新 mpgroup 表失败');
                    }
    
                    $resultTag = Db::name('mpgrouptag')
                        ->whereRaw("FIND_IN_SET(?, access)", [$chat_id])
                        ->exp('access', "REPLACE(access, '{$chat_id}', '{$channelId}')")
                        ->update();
    
                    if ($resultTag === false) {
                        throw new \Exception('更新 mpgrouptag 表失败');
                    }
                    ...
                    $tasks = Db::name('mprenwu')
                        ...
                        ->where(function ($query) use ($channelId) {
                            $query->whereRaw("FIND_IN_SET(?, mgt.access)", [$channelId]);
                        })
                        ->select();
                    Cache::store('redis')->set($cacheKey, $tasks, 3600);
    
                    Db::commit();
                    //Log::info("数据库更新及缓存重新生成成功");
                } catch (\Exception $e) {
                    Db::rollback();
                   // Log::error("数据库更新及缓存重新生成失败：" . $e->getMessage());
                }
                
            }
             // 判断是否为用户加入群组的消息
            $isUserJoined = isset($message['action']) && $message['action']['_'] === 'messageActionChatJoinedByLink';
            //推送规则
            // 判断群组是否为英文群组
            if ($this->isEnglishGroup($chat_id, $accountName, $instance)) {
                  // 任务生成缓存键
                    ...
                    if (!$tasks) {
                        ...
                        Cache::store('redis')->set($cacheKey, $tasks, 3600);
                            ...
                            // 记录错误日志
                            Log::error('查询任务时出错: '. $e->getMessage());
                            $tasks = []; // 出错时返回空数组
                        }
                    }
                     // 判断昵称是否为中文
                ...
                foreach ($tasks as $task) {
                    if (isset($task['access']) && in_array($chat_id, explode(',', $task['access']))) {
                        $filteredTasks[] = $task;
                    }
                }
                    foreach ($filteredTasks as $task) {
                        ...
                        if($filterType=='user_id'){
                            ...
                            if ($this->containsKeywords($text, $filterValue)) {
                                $ispushm=true;
                                $ispushm_chatid=$task['push_chatid'];
                            }
                        }elseif($filterType=='newuser'){
                            if ($isUserJoined) {
                                $ispushm=true;
                                $ispushm_chatid=$task['push_chatid'];
                            }
            ...
            if ($ispushm&&$ispushm_chatid) {
                // 生成 Redis 缓存键
                $redisKey = "user_recorded_{$user_id}";
                ...
                if ($userRecordCache && $userRecordCache['has_message_text'] &&!$userRecordCache['is_pushed']) {
                    ...
                    if ($firstMessageInfo) {
                       ...
                       // $pushContent.= "用户名：{$firstMessageInfo['username']}\n";
                        ...
                        $pushContent.= "时间：{$today}\n";
                        $pushContent.= "{$firstMessageInfo['message_text']}\n";
    ...
    { 
        ...
        $message = $update['message'];
        $user_id = $message['from_id'] ?? null;
        if (isset($message['peer_id']) &&$user_id >0) {
            // 获取频道ID
           ...
           // Log::info(json_encode($userInfo, JSON_PRETTY_PRINT));
            ...
            // 获取消息文本，如果消息文本不存在，使用默认提示
            $text = $message['message'] ?? ($isUserJoined ? '用户通过链接加入群组' : '无有效消息内容');
            $messageId = $message['id'] ?? uniqid();
            // 生成消息链接
            $message_link = $this->getMessageLink($chat_id, $messageId, $accountName, $instance,2);
            // 存储用户信息到数据库
            $isalluser=$this->saveUserInfo($user_id,  $userInfo['username'], $userInfo['nickname'], $text, null, $message_link, $accountName);
            // 保存群组信息到数据库
            ...
            if ($this->isEnglishGroup($chat_id, $accountName, $instance)) {
                ...
                // log::info('开始创建ispushm'.$ispushm_chatid);
                // 检查发言是否包含关键词
            if ($ispushm&&$ispushm_chatid) {
...
private function saveUserInfo($user_id, $username, $user_nickname, $message_text, $new_member_info, $message_link, $accountName)
{
    ...
    $existingUser = null;
    if (!$userRecordCache) {
        $membeUser = Db::name('membe')
            ->where('user_id', $user_id)
            ->field('user_id,membe_id')
            ->find();
        if($membeUser){
        ...
        if (!$existingUser) {
            ...
            if (empty($existingUser['message_text']) &&!empty($message_text)) {
                $this->updateUserMessageText($user_id, $accountName, $message_text, $message_link, $isPushed);
...
private function updateUserMessageText($user_id, $accountName, $message_text, $message_link, $isPushed)
{
    ...
    private function saveGroupInfo($group_id, $accountName, $instance)
    {
        ...
        if (!$groupInfo) {
            ...
            // 先查询数据库中是否已经存在该群组信息
            $existingGroup = Db::name('mpgroup')->where('group_id', $group_id)->where('phone', $accountName)->find();
            if ($existingGroup) {
                // 若存在，则更新群组名称和类型
               /* Db::name('mpgroup')->where('group_id', $group_id)->update([
                ...
                Cache::store('redis')->set($cacheKey, $newGroupInfo, 3600);
            } else {
               $adminId=$this->getMpadmininfo($accountName);
                // 若不存在，则插入新的群组信息
    ...
    private function getMessageLink($chat_id, $message_id, $accountName, $instance,$type)
    {
        ...
        if ($group_username !== 'Unknown Username') {
            ...
            return "https://t.me/c/" . abs($chat_id) . "/{$message_id}";
        }
   ...
   /**
    ...
    private function pushMessage($targetGroupId, $text, $message_link, $accountName)
    {
       ...
       // log::info('我要推送了');
        ...
        $mptoken = config('app.mptoken');
            // 发送消息
       $rs= send($mptoken, 'sendMessage', $postData);
        // log::info($rs);
    }
